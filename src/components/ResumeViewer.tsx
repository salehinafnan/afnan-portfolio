...
const Page = ({ pdf, index, page, width }: PageProps) => {
  ...
  return (
    ...
    >
      {!painted && <div aria-hidden="true" className="absolute inset-0 animate-pulse bg-stone-200" />}
      ...
      {page.links.map((link, i) => {
        ...
        return (
          <a
            ...
            title={link.label}
            style={link.box}
            className="absolute rounded-xs transition-colors hover:bg-indigo-500/15 focus-visible:bg-indigo-500/15"
          />
        );
      })}
    </div>
...
const iconButton =
  "flex size-10 shrink-0 items-center justify-center rounded-full text-lg text-stone-600 transition-colors hover:bg-stone-100 hover:text-stone-950";
...
const ResumeViewer = () => {
  ...
  return (
    ...
    >
      <header className="flex shrink-0 items-center gap-1 border-b border-stone-200 bg-surface py-3 pr-3 pl-5 sm:gap-2 sm:pl-6">
        <div className="mr-2 min-w-0 grow">
          <h2 id="cv-title" className="truncate font-semibold tracking-tight text-stone-950">
            Curriculum Vitae
          </h2>
        ...
        <a
          ...
          className="flex h-10 shrink-0 items-center gap-2 rounded-full bg-stone-950 px-3 text-sm font-medium text-white transition-colors hover:bg-stone-800 sm:px-4"
        >
        ...
        <div ref={pagesRef} className="mx-auto max-w-4xl space-y-4 sm:space-y-6">
          {resume ? (
            ...
            <div role="alert" className="flex flex-col items-center gap-4 py-24 text-center">
              <FiAlertCircle aria-hidden="true" className="size-8 text-stone-400" />
              <p className="text-sm text-stone-600">The CV couldn&apos;t be loaded here.</p>
              <div className="flex flex-wrap justify-center gap-3">
                ...
                <a
                  ...
                  className="rounded-full border border-stone-200 bg-surface px-5 py-2.5 text-sm font-medium text-stone-900 transition-colors hover:border-stone-300"
                >
            ...
            <div role="status" className="aspect-210/297 animate-pulse rounded-sm bg-stone-200">
